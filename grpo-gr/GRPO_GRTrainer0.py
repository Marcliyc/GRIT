...
parent_dir = os.path.dirname(current_dir) # This should be .../grit/GRIT/

# Add it to Python's search path
if parent_dir not in sys.path:
    sys.path.append(parent_dir)

# Dynamic import if you have different tools, OR specific import if it's always one tool:
from custom_tools.bbox_brush.bbox_brush import BoundingboxBrush
...
class GRPOGRTrainer(Trainer):
    ...
    def __init__(
        ...
        for tool_name in tool_names:
            ...
            #self.tools[tool_func] = load_tool(f"./custom_tools/{tool_path}", port=self.process_uinque_id, project_root_path=self.args.project_root_path, python_path_for_dino=self.args.python_path_for_dino)#,trust_remote_code=True)
            self.tools[tool_func] = BoundingboxBrush(port=self.process_uinque_id, project_root_path=self.args.project_root_path, python_path_for_dino=self.args.python_path_for_dino)
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]], is_train = True) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        while True:
            ...
            if "qwen" in self.model_id.lower():
                ...
                with torch.no_grad():
                    ...
                    # Note: prompt_inputs.input_ids comes from the processor we used earlier
                    ...
                    if tool_call_query is None:
                        ...
                        if self.args.log_completions and 'eval' in inputs[i] and inputs[i]['eval']:
                            ...
                            try:
                                ...
                                image_base64, response = self.tools[self.tool_func](image_base64 = img_str, bbox = tool_call_query, normalized_bboxs = True if 'internvl' in self.model_id else False) #self.tool_processor, self.tool_model,
        ...
        for i, reward_func in enumerate(self.reward_funcs):
            ...
            if "grounded_region_specific_thinking_format_reward" in reward_func_name:
                ...
                idx_gpt_reward = i
            # elif 'answer_correctness_reward' in reward_func_name:
            #     idx_correctness_reward = i
        ...
        if rewards.shape[0] % self.num_generations == 0:
        # Compute grouped-wise rewards
            ...
            advantages = (rewards - mean_grouped_rewards) / (std_grouped_rewards + 1e-4)
                ...
                print(f'Unexpected: Incompatible reward shape {rewards.shape} and num_gen {self.num_generations}')
                # Fallback for the final odd-sized batch (e.g., size 11 with groups of 4)
                n_gen = self.num_generations
                n_full = rewards.shape[0] // n_gen
                remainder = rewards.shape[0] % n_gen
                means_list = []
                std_list = []
                repeat_counts = []
                
                # 1. Process complete groups
                if n_full > 0:
                    full_part = rewards[:n_full*n_gen]
                    means_list.append(full_part.view(-1, n_gen).mean(dim=1))
                    std_list.append(full_part.view(-1,n_gen).std(dim=1))
                    repeat_counts.extend([n_gen] * n_full)
                    
                # 2. Process the leftover group
                if remainder > 0:
                    rem_part = rewards[n_full*n_gen:]
                    means_list.append(rem_part.mean().view(1))
                    std_list.append(rem_part.std().view(1))
                    repeat_counts.append(remainder)
                
                # 3. Combine
                mean_grouped_rewards = torch.cat(means_list)
                std_grouped_rewards = torch.cat(std_list)
                
                # 4. Expand correctly (Complete groups get 4 repeats, Remainder gets 'remainder' repeats)
                expanded_means = torch.repeat_interleave(
                    mean_grouped_rewards, 
                    torch.tensor(repeat_counts, device=rewards.device)
                )
                expanded_std = torch.repeat_interleave(
                    std_grouped_rewards, 
                    torch.tensor(repeat_counts, device=rewards.device)
                )
                
                advantages = (rewards - expanded_means) / (expanded_std + 1e-4)
